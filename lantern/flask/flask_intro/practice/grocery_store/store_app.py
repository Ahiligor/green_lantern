...
class NoSuchManagerIDError(Exception):
    def __init__(self, manager_id):
        self.message = f'No such store_id {manager_id}'
...
@app.errorhandler(NoSuchManagerIDError)
def my_error_handler(e):
    return jsonify({'error': e.message}), 404
...
def create_good():
    db = inject.instance('DB')
    good_qty = db.goods.add(request.json)
    return jsonify({'numbers of items created': good_qty}), 201
...
def update_goods():
    ...
    updated = db.goods.update_goods(request.json)
